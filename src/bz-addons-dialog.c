...
{
  const char *tag           = adw_navigation_view_get_visible_page_tag (self->navigation_view);
  int         target_width  = 0;
  int         target_height = 0;
  int         nat           = 0;
  int         cur_width     = 0;
  ...
  if (g_strcmp0 (tag, "list") == 0)
  ...
  else if (g_strcmp0 (tag, "full-view") == 0)
    {
      ...
      target_height = CLAMP (nat + 70, 300, 700);
    }
                               ...
                               GWeakRef  *wr)
{
  ...
  if (value != NULL && G_VALUE_HOLDS_OBJECT (value))
    {
      g_autoptr (BzEntry) ui_entry = g_value_dup_object (value);
      ...
      if (BZ_IS_FLATPAK_ENTRY (ui_entry))
        {
          const char *ref = bz_flatpak_entry_get_addon_extension_of_ref (BZ_FLATPAK_ENTRY (ui_entry));
                  ...
                  if (parent_group != NULL)
                    {
                      ...
                      if (self->parent_ui_entry != NULL &&
                          bz_result_get_resolved (self->parent_ui_entry))
                        {
                          g_object_notify_by_pspec (G_OBJECT (self), props[PROP_PARENT_UI_ENTRY]);
                        }
                      else if (self->parent_ui_entry != NULL)
                        {
  ...
  if (group != NULL)
    {
      ...
      if (self->selected_ui_entry != NULL &&
          bz_result_get_resolved (self->selected_ui_entry))
        {
      ...
      else if (self->selected_ui_entry != NULL)
        {
...
bz_addons_dialog_new (BzEntryGroup *group)
{
  ...
  return ADW_DIALOG (g_object_new (
