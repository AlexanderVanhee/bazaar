...
static gboolean
ptr_array_contains_str (GPtrArray  *arr,
                        const char *needle)
{
...
static void
ptr_array_remove_str (GPtrArray  *arr,
                      const char *needle)
{
  for (guint i = 0; i < arr->len; i++)
    {
                    ...
                    const char *value)
{
  ...
  return strv_contains ((const char *const *) values, value);
}
  ...
  for (guint i = 0; entries[i] != NULL; i++)
    {
      ...
      g_clear_pointer (&entry_key, g_free);
    }
      ...
      for (guint i = 0; ovr_values[i] != NULL; i++)
        {
          ...
          if (val[0] == '!')
            ptr_array_remove_str (paths, val + 1);
          else if (!is_well_known_filesystem (val) &&
                   !ptr_array_contains_str (paths, val))
      ...
      for (guint i = 0; ovr_keys[i] != NULL; i++)
        {
          ...
          if (g_strcmp0 (val, policy_type) == 0)
            {
              if (!ptr_array_contains_str (names, ovr_keys[i]))
                g_ptr_array_add (names, g_strdup (ovr_keys[i]));
            }
          else if (g_strcmp0 (val, "none") == 0)
            {
              ptr_array_remove_str (names, ovr_keys[i]);
            }
        }
    }
...
load_all_entry_row_states (BzPermissionsPage *self)
{
  ...
  load_bus_policy_state (self, self->session_bus_talk,
                         "Session Bus Policy", "talk");
  load_bus_policy_state (self, self->session_bus_own,
                         "Session Bus Policy", "own");
  load_bus_policy_state (self, self->system_bus_talk,
                         "System Bus Policy", "talk");
  load_bus_policy_state (self, self->system_bus_own,
                         "System Bus Policy", "own");
...
save_overrides (BzPermissionsPage *self)
{
        ...
        for (guint i = 0; meta_filesystems[i] != NULL; i++)
          {
            ...
            if (!strv_contains ((const char *const *) fs_values,
                                meta_filesystems[i]))
  ...
  {
    BusPolicyMapping bus_entries[] = {
      { self->session_bus_talk, "Session Bus Policy", "talk" },
      {  self->session_bus_own, "Session Bus Policy",  "own" },
      {  self->system_bus_talk,  "System Bus Policy", "talk" },
      {   self->system_bus_own,  "System Bus Policy",  "own" },
    };

    for (guint b = 0; b < G_N_ELEMENTS (bus_entries); b++)
      {
        g_auto (GStrv) current_names = NULL;
        g_auto (GStrv) meta_keys     = NULL;

        current_names = bz_permission_entry_row_get_values (bus_entries[b].entry_row);
        meta_keys     = g_key_file_get_keys (self->metadata,
                                             bus_entries[b].kf_group,
                                             NULL, NULL);
            ...
            for (guint i = 0; current_names[i] != NULL; i++)
              {
                ...
                if (g_strcmp0 (meta_val, bus_entries[b].policy_type) == 0)
                  continue;

                g_key_file_set_string (kf, bus_entries[b].kf_group,
                                       current_names[i],
                                       bus_entries[b].policy_type);
              }
          }
            ...
            for (guint i = 0; meta_keys[i] != NULL; i++)
              {
                ...
                if (!strv_contains ((const char *const *) current_names,
                                    meta_keys[i]))
                  g_key_file_set_string (kf, bus_entries[b].kf_group,
                                         meta_keys[i], "none");
              }
...
{
  BusPolicyMapping bus_entries[] = {
    { NULL, "Session Bus Policy", "talk" },
    { NULL, "Session Bus Policy",  "own" },
    { NULL,  "System Bus Policy", "talk" },
    { NULL,  "System Bus Policy",  "own" },
  };

  if (self->entry_group == NULL)
    return;

  bus_entries[0].entry_row = self->session_bus_talk;
  bus_entries[1].entry_row = self->session_bus_own;
  bus_entries[2].entry_row = self->system_bus_talk;
  bus_entries[3].entry_row = self->system_bus_own;
  ...
  for (guint b = 0; b < G_N_ELEMENTS (bus_entries); b++)
    {
      load_bus_policy_state (self, bus_entries[b].entry_row,
                             bus_entries[b].kf_group,
                             bus_entries[b].policy_type);
      g_signal_connect (bus_entries[b].entry_row, "changed",
                        G_CALLBACK (entry_row_changed_cb), self);
    }
